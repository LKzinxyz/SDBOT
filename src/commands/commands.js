const handleCommands = (bot, channel, username, message) => {
  ...
  // Envia mensagem ou comandos no chat
  ...
  // Anuncia a cada 30 minutos 
    ...
    if (split[1] == bot.username || split[1] == 'all') {
      if (bot.location !== 'home') {
        ...
        split = split.splice(2, index)
        split = split.join(' ')
        setInterval(_ => bot.chat(split), 1800000)
        setInterval(_ => channel.send(`${username} Anuncio Enviado! `), 1800000)
      }
    }
  }

  // Pega Kit Mensal
  else if (message.content.startsWith(`+kitmensal`)) {
    let split = message.content.split(' ')
    if (split[1] == bot.username || split[1] == 'all') {
      if (bot.location !== 'home') {
        channel.send(`${username} não está na home, aguarde um momento`)
        ...
        split = split.splice(2, index)
        split = split.join(' ')
        channel.send(`${username}: Pegou o kit mensal.`)
        bot.chat('/kit')
        bot.on('windowOpen', function () {
